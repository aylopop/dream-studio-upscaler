...
def set_config(req: dict[str, Any], is_api=False, run_callbacks=True, save_config=True):
    ...
    for k, v in req.items():
        ...
        if k == "sd_model_checkpoint":
            ...
            # refresh=True so refresh_model_loading_parameters actually
            # updates model_data.forge_loading_parameters — otherwise the
            # next manage_model_and_prompt_cache reads stale params and
            # keeps the OLD model in VRAM even though opts.data shows the
            # new value (the "GUI says A, Forge has B in VRAM" bug for
            # override_settings sd_model_checkpoint switches).
            checkpoint_changed = main_entry.checkpoint_change(v, preset=None, save=False, refresh=True)
