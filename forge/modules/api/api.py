...
class Api:
    def __init__(self, app: FastAPI, queue_lock: Lock):
        ...
        # Vendored interrogate — Forge Neo dropped the upstream endpoint.
        # Backed by clip-interrogator (BLIP+CLIP). See modules/interrogate_neo.py.
    ...
    def interrogateapi(self, req: dict):
        # Lazy import so Forge startup doesn't pay the clip-interrogator
        # import cost when nobody's using the feature.
        from modules import interrogate_neo
        image = req.get("image", "")
        clip_model = req.get("clip_model_name") or interrogate_neo.DEFAULT_CLIP_MODEL
        mode = req.get("mode", "fast")
        if not image:
            return {"caption": ""}
        try:
            caption = interrogate_neo.interrogate(image, clip_model_name=clip_model, mode=mode)
        except Exception as e:
