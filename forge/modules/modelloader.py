...
def load_upscalers():
    ...
    esrgan_path = shared.cmd_opts.esrgan_models_path
    esrgan = UpscalerESRGAN(esrgan_path)
    esrgan.user_path = esrgan_path
    esrgan.model_download_path = esrgan_path or esrgan.model_path

    dat_path = getattr(shared.cmd_opts, "dat_models_path", None)
    extra_scalers = []
    if dat_path:
        # Skip silently if the directory isn't present — keeps existing
        # deployments that don't have a DAT folder working as before.
        dat = UpscalerDAT(dat_path)
        extra_scalers.extend(dat.scalers)

    shared.sd_upscalers = [
        *UpscalerNone().scalers,
        *UpscalerLanczos().scalers,
        *UpscalerNearest().scalers,
        *sorted([*esrgan.scalers, *extra_scalers], key=lambda s: s.name.lower()),
    ]
